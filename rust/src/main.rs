...
fn ASSERT_LIGHT_COLOR(sm: &LightSm::LightSm, expected: String) {
...
fn test_inc_events() {
    ...
    // LightSm_ctor(&sm);
    let mut sm: LightSm::LightSm = LightSm::LightSm::default();
    sm.start();

    ASSERT_LIGHT_COLOR(&sm, "off".into());

    sm.dispatch_event(LightSm::EventId::INC);
...
fn test_dim_events() {
    ...
    sm.dispatch_event(LightSm::EventId::DIM);
    ASSERT_LIGHT_COLOR(&sm, "off".into());
}
...
fn test_off_events() {
    ...
    sm.dispatch_event(LightSm::EventId::OFF);
    ASSERT_LIGHT_COLOR(&sm, "off".into());
}
